...
namespace AOISystem.Utilities.Modules.Syntek.EtherCAT.SlaveModules.Motion
{
    ...
    internal partial class CEtherCATMotionForm : Form
    {
        ...
        private CEtherCATMotionForm()
        {
            InitializeComponent();
        }

        public CEtherCATMotionForm(CEtherCATMotion axis, Language language)
        {
            InitializeComponent();
        ...
        public CEtherCATMotionForm(CEtherCATMotion axis)
        {
            InitializeComponent();
            this.Text = axis.DeviceName;
            this.axis = axis;
        ...
        {
            ntxbStrVel.Text = axis.AxisPara.StrVel.ToString();
            ntxbConstVel.Text = axis.AxisPara.ConstVel.ToString();
            ntxbEndVel.Text = axis.AxisPara.EndVel.ToString();
            ntxbTAcc.Text = axis.AxisPara.TAcc.ToString();
            ntxbTDec.Text = axis.AxisPara.TDec.ToString();
            ntxbAccVel.Text = axis.AxisPara.AccVel.ToString();
            ntxbDecVel.Text = axis.AxisPara.DecVel.ToString();
            ntxbHomeMode.Text = axis.AxisPara.HomeMode.ToString();
            ntxbHomeOffset.Text = axis.AxisPara.HomeOffset.ToString();
            ntxbHomeSwitchVel.Text = axis.AxisPara.HomeSwitchVel.ToString();
            ntxbHomeZeroVel.Text = axis.AxisPara.HomeZeroVel.ToString();
            ntxbHomeAccVel.Text = axis.AxisPara.HomeAccVel.ToString();
            ...
            cmbEncMode.SelectedIndex = Convert.ToInt16(axis.AxisPara.EncMode);
            cmbLogicORG.SelectedIndex = Convert.ToInt16(axis.AxisPara.LogicORG);
            cmbLogicSvon.SelectedIndex = Convert.ToInt16(axis.AxisPara.LogicSvon);
            cmbLogicZ.SelectedIndex = Convert.ToInt16(axis.AxisPara.LogicZ);
            cmbLogicEL.SelectedIndex = Convert.ToInt16(axis.AxisPara.LogicEL);
            ...
            lblCardID.Text = string.Format("Card ID : {0}", axis.AxisPara.CardNo);
            lblNodeID.Text = string.Format("Node ID : {0}", axis.AxisPara.NodeNo);
            lblSlotID.Text = string.Format("Slot No : {0}", CEtherCAT.g_uESCSlotID);

            this.btnServoON.Text = axis.Status.SwitchOn ? "Servo OFF" : "Servo ON";

            this.grpR1EC5621.Enabled = axis.MotionModel == MotionModel.SYNTEK_R1_EC5621;
        }
                    ...
                    if (((NumTextBox)sender).Tag.ToString() == p.Name)
                    {
                        ...
                        if (p.PropertyType.BaseType.Name == "ValueType")
                        {
                            try
                            {
                                ...
                                PropertyHelper.PropertyInfoSetValue(axis.AxisPara, p, val);
                            }
                            catch (Exception)
                            {
                            }
        ...
        private void btn60FD_Click(object sender, EventArgs e)
        {
            axis.SDO_60FD();
        }
        ...
        {
            MessageBox.Show(axis.GetErrorCode());
        }
    }
}