...
{
    //此Form是從L132M1X1Form來的，許多M314才有的功能並未在此Form上實做。
    //導程 : 螺桿(馬達)轉一圈的行程(長度) (線馬導程應該都是1)
    //每轉pulse量 : 馬達轉一圈所需的pulse(透過Driver設定 通常1pulse = 1um)
    internal partial class M314Form : Form
    {
        ...
        public M314Form(M314 axis , Language language)
        {
            InitializeComponent();
        ...
        private void M314Form_Load(object sender , EventArgs e)
        {
            parameterInitial();
        ...
        private void M314Form_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void tmrParaScan_Tick(object sender , EventArgs e)
        {
            ...
            {
                sw.Stop();
                speed = (axis.Encoder - position) / sw.ElapsedMilliseconds * 1000;
            }
            position = axis.Encoder;
            sw.Restart();
            ...
            if (sampleCount == 0)
            {
                this.label30.Text = string.Format("{0:F2}, {1:F2}", maxSpeed, minSpeed);
